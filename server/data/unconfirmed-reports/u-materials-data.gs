...
function doPost(request) {
	...
	const { date, time, responsible, objects, stages, materials, quantity, prices, sums, comments, edit, remove } = request.parameter;
	...
	const reportNumber = `M-22-${Number(lastReportComponents[2]) + 1}`;
	...
	for (let i = 0; i < quantityList.length; i++) {
		...
		sheet.getRange(`H${lastRow + 1 + i}`).setValue(materialsList[i]);
